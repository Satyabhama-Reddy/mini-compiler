...
int csym=0;
char add[50];
int beg=0,w=0,temp=0,cbeg=0,cond=0;
int yyerror();  
int yylex();
%}
...
primary_expression
	...
	| '(' {l++;expr=l+1;} expression ')' {if(cond==1){strcpy($$.addr,$3.addr);sprintf($$.code,"%s",$3.code);}
																		else{strcpy($$.addr,$3.addr);strcpy($$.code,$3.code);}
																		}
	...
	| logical_and_expression T_AND_OP equality_expression {
					...
					if($<node>0.begin!=0){temp=t;sprintf($$.code,"%sifFalse %s goto END%d\n%s",$1.code,$1.addr,$<node>0.begin,$3.code);}
   					...
   					if(strcmp($<node>0.next,"")==0){
   						...
   						if($<node>0.begin!=0){temp=t;sprintf($$.code,"%sifFalse %s goto END%d\n%s",$1.code,$1.addr,$<node>0.begin,$3.code);}
   						printf("code:\n\n\n%s\n%d\n\n",$$.code,expr);
   						if(expr!=0){sprintf($$.code,"%sifFalse %s goto L%d\n%s",$1.code,$1.addr,expr,$3.code);strcpy($$.addr,$3.addr);}
   					}
	;
...
logical_or_expression
	: logical_and_expression {
			...
			strcpy($$.code,$1.code);
			printf("here");
			if(expr!=0){sprintf($$.code,"%sL%d : ",$1.code,expr	);expr=0;}
			//strcpy($$.next,$1.next);
			}
	| logical_or_expression T_OR_OP {strcpy($<node>$.next,$<node>0.next);} logical_and_expression {temp=t;
   			...
   			printf("code:\n\n\n%s\n%d\n\n",$$.code,expr);
   			if(expr!=0){sprintf($$.code,"%sifFalse %s goto L%d\nL%d : %s",$1.code,$1.addr,expr,expr,$4.code);strcpy($$.addr,$4.addr);expr=0;}}
	;
...
X : T_IF '(' {cond=1;l++;sprintf($<node>$.true,"L%d",l);l++;sprintf($<node>$.next,"L%d",l);$<node>$.begin=0;} logical_or_expression ')' {
						if(combined==1) sprintf($$.code,"%sifFalse t%d goto %s\n%s : ",$4.code,t,$<node>3.next,$<node>3.true);
...
selection_statement
	: X statement {sprintf($$.code,"%s%s%s:",$1.code,$2.code,$$.next);} %prec IFX 
	| X statement T_ELSE statement {l++;sprintf($$.code,"%s%sgoto L%d\n%s:%sL%d:",$1.code,$2.code,l,$$.next,$4.code,l);}
	;

while_selection_statement
	: X while_statement {sprintf($$.code,"%s%s%s:",$1.code,$2.code,$$.next);} %prec IFX 
	| X while_statement T_ELSE while_statement {l++;sprintf($$.code,"%s%sgoto L%d\n%s:%sL%d:",$1.code,$2.code,l,$$.next,$4.code,l);}
	;

iteration_statement
	: T_WHILE '(' {cond=1;beg++;cbeg++;$<node>$.begin=beg;w=1;sprintf($<node>$.next,"END%d",beg);l++;sprintf($<node>$.true,"L%d",l);} logical_or_expression {cond=0;} ')' while_statement {cbeg--;
			if(combined==1) sprintf($$.code,"BEGIN%d : %sifFalse %s goto END%d\n%s : %sgoto BEGIN%d\nEND%d : ",$<node>3.begin,$4.code,$4.addr,$<node>3.begin,$<node>3.true,$7.code,$<node>3.begin,$<node>3.begin);
			else sprintf($$.code,"BEGIN%d : %sifFalse %s goto END%d\n%sgoto BEGIN%d\nEND%d : ",$<node>3.begin,$4.code,$4.addr,$<node>3.begin,$7.code,$<node>3.begin,$<node>3.begin);
			strcpy($$.next,$<node>3.next);
			combined=0;w=0;
			//printf("\n\n%s\n\n",$4.code);
			}
...
jump_statement
	...
	| T_RETURN ';' {sprintf($$.code,"goto NEXT\n");}
	| T_RETURN logical_or_expression ';'  {sprintf($$.code,"goto NEXT\n");}
	;
